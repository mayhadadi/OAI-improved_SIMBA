"""
GFCS-Adaptive v3: Gradient First, Coimage Second with Adaptive Improvements
============================================================================
Enhanced implementation of GFCS with two key improvements:

1. ADAPTIVE SURROGATE WEIGHTING (Improvement #3):
   - Use weighted combination of ALL surrogate gradients (like averaged_gradients)
   - Track gradient alignment with successful update direction
   - Increase trust for surrogates whose gradients aligned with success
...
v3 Changes (fixing v2 performance regression):
- Back to weighted combination (not individual surrogate testing)
- Infer surrogate contribution by gradient alignment with successful direction
- Follows original GFCS flow: try combined gradient, fall back to ODS
...
class GFCSWeightedAlignment:
    ...
    def attack(
        ...
        """
        Run GFCS-Adaptive v3 attack.
        
        Algorithm:
        1. Get gradients from all surrogates
        2. Compute weighted combination based on trust
        3. Try SimBA update with combined gradient
        4. If success: update trust based on alignment
        5. If fail: fall back to smart ODS
        6. Repeat until adversarial or max queries
        """
